...
void pcdCallback(const pcl::PointCloud<PointT> msg)
{
  ...
  pcl::PointCloud<PointT>::Ptr cloud = &msg;
  ...
  {
    // Loop for the number of points we have per row.
    for(unsigned int j = 0; j < cloud->width; j++)
    {
      ...
      if(!((cloud->points[i*cloud->width + j].x != cloud->points[i*cloud->width + j].x) ||
           (cloud->points[i*cloud->width + j].y != cloud->points[i*cloud->width + j].y) ||
           (cloud->points[i*cloud->width + j].z != cloud->points[i*cloud->width + j].z)))
      {
        // Grab the x y and z values and pad the middle with junk for the pts file.
        pts_file << cloud->points[i*cloud->width + j].x << " ";
        pts_file << cloud->points[i*cloud->width + j].y << " ";
        pts_file << cloud->points[i*cloud->width + j].z << " ";
        pts_file << 0 << " ";
        ...
        vrml_file << cloud->points[i*cloud->width + j].x << " ";
        vrml_file << cloud->points[i*cloud->width + j].y << " ";
        vrml_file << cloud->points[i*cloud->width + j].z << ",\n";
      }
    }
  }
...
int main(int argc, char **argv)
{
  ...
  // Create a subscriber on the depth topic.
  ros::Subscriber sub = n.subscribe("transformed_cloud", 1000, pcdCallback);
